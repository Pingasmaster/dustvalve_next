...
#   ./build.sh --smoke            # Tier 2 on-device smoke on GMD pixel6Api37 + exit
#   ./build.sh --e2e              # Tier 3 hermetic E2E on GMD pixel6Api37 + exit
...
if [[ "$DO_SMOKE" -eq 1 ]]; then
    acquire_lock
    ./gradlew :app:pixel6Api37Setup "$GMD_GPU"
    ./gradlew :app:pixel6Api37DebugAndroidTest "$GMD_GPU" \
