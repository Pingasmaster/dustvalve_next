...
class YouTubeSourceTest {
    ...
    @Test fun `getArtist returns metadata with empty albums`() = runTest {
        ...
        } returns YouTubeChannelResult(emptyList(), "The Channel", null, avatarUrl = "https://img/avatar")
        ...
        assertThat(artist.imageUrl).isEqualTo("https://img/avatar")
    ...
    @Test fun `getArtistTracks passes the opaque continuation through`() = runTest {
        ...
        } returns YouTubeChannelResult(tracks, "Artist", nextToken)
    ...
    @Test fun `getArtistTracks reports no more when continuation is null`() = runTest {
        ...
        } returns YouTubeChannelResult(listOf(track("a")), "Artist", null)
    ...
    @Test fun `getCollection surfaces playlist coverUrl from the repository`() = runTest {
        val tracks = listOf(track("a"), track("b"))
        coEvery { ytRepo.getPlaylistTracks("https://pl") } returns YouTubePlaylistResult(
            tracks = tracks,
            title = "My List",
            coverUrl = "https://i.ytimg.com/vi/abc/hq720.jpg",
        )

        val collection = source.getCollection("https://pl")
        assertThat(collection.coverUrl).isEqualTo("https://i.ytimg.com/vi/abc/hq720.jpg")
    }

    @Test fun `getCollection falls back to first-track art when cover is missing`() = runTest {
        val tracks = listOf(
            track("a").copy(artUrl = "https://i.ytimg.com/vi/a/hq720.jpg"),
            track("b"),
        )
        coEvery { ytRepo.getPlaylistTracks("https://pl") } returns YouTubePlaylistResult(
            tracks = tracks,
        ...
        val collection = source.getCollection("https://pl")
        assertThat(collection.coverUrl).isEqualTo("https://i.ytimg.com/vi/a/hq720.jpg")
    }
