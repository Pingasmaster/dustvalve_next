...
class CookieStoreTest {
    ...
    @Test fun `loadForRequest filters host`() = runBlocking {
        ...
        val cookie = Cookie.Builder()
            .name("session").value("x").domain("bandcamp.com").path("/").build()
    ...
    @Test fun `clearCookiesForDomain removes matching only`() = runBlocking {
        store.saveFromResponse(
            "https://bandcamp.com/".toHttpUrl(),
            listOf(Cookie.Builder().name("k").value("v").domain("bandcamp.com").path("/").build()),
        )
        store.saveFromResponse(
            "https://youtube.com/".toHttpUrl(),
            listOf(Cookie.Builder().name("yk").value("yv").domain("youtube.com").path("/").build()),
        )
    ...
    @Test fun `loadForRequest omits secure cookies on plain http`() = runBlocking {
        val cookie = Cookie.Builder()
            .name("session").value("x").domain("bandcamp.com").path("/").secure().build()
        store.saveFromResponse("https://bandcamp.com/".toHttpUrl(), listOf(cookie))
    ...
    @Test fun `cookies saved before init completes are not clobbered by the disk load`() {
        // Regression: the async init used to unconditionally assign the disk
        // snapshot to the cache, wiping cookies saved during startup.
        ...
        every { spySettings.authCookies } returns flow {
            gate.await() // hold the disk load until the save below has run
            emit(diskJson)
        }
        val racyStore = CookieStore(spySettings, Dispatchers.IO)

        val cookie = Cookie.Builder()
            .name("new").value("nv").domain("bandcamp.com").path("/").build()
        racyStore.saveFromResponse("https://bandcamp.com/".toHttpUrl(), listOf(cookie))
        gate.countDown()
