...
fun AlbumEntity.toDomain(tracks: List<Track>, isFavorite: Boolean): Album = Album(
    ...
    // Local captures: entity properties live in :core:database now, so
    // cross-module smart casts on them are not allowed.
    purchaseInfo = run {
        val itemId = saleItemId
        val itemType = saleItemType
        if (itemId != null && itemType != null) PurchaseInfo(itemId, itemType) else null
    },
    discographyOffer = run {
        val amount = discogPriceAmount
        val currency = discogPriceCurrency
        val offerUrl = discogUrl
        if (amount != null && !currency.isNullOrBlank() && !offerUrl.isNullOrBlank()) {
            DiscographyOffer(
                price = AlbumPrice(amount = amount, currency = currency),
                url = offerUrl,
                name = discogName ?: "",
            )
        } else {
    ...
    singleTrackPrice = run {
        val amount = singleTrackPriceAmount
        val currency = singleTrackPriceCurrency
        if (amount != null && !currency.isNullOrBlank()) AlbumPrice(amount = amount, currency = currency) else null
    },
)
