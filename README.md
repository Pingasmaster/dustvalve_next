...
- Play music from **YouTube** via NewPipe Extractor — no API key or Google account needed
- Scan and play **local audio files** (mp3, flac, m4a, ogg, wav, opus, aac, wma, alac)

**Player**
- Queue management with shuffle and repeat (off / all / one)
- Volume control, media session notifications, and lock screen controls
- 512 MB LRU media cache for seamless playback
...
```bash
git clone https://github.com/example/dustvalve-next.git
cd dustvalve-next
./build.sh
```
